...
func (s *TicketRepoTestSuite) TestCreateTicket(t provider.T) {
	...
	s.mock.ExpectQuery(regexp.QuoteMeta(
		...
		WithArgs(ticket.TicketUID, ticket.Username, ticket.FlightNumber, ticket.Price, ticket.Status, ticket.ID).
...
func (s *TicketRepoTestSuite) TestUpdateTicket(t provider.T) {
	...
	s.mock.ExpectQuery(regexp.QuoteMeta(
		...
		WithArgs(ticket.TicketUID, ticket.Username, ticket.FlightNumber, ticket.Price, ticket.Status, ticket.ID).WillReturnRows(rows)
...
func (s *TicketRepoTestSuite) TestGetAll(t provider.T) {
	...
	for i := range tickets {
		rowsTickets.AddRow(tickets[i].ID, tickets[i].TicketUID, tickets[i].Username, tickets[i].FlightNumber, tickets[i].Price, tickets[i].Status)
	}
...
func (s *TicketRepoTestSuite) TestGetAllByUserName(t provider.T) {
	tickets := make([]models.Ticket, 10)
	for _, ticket := range tickets {
		err := faker.FakeData(&ticket)
		t.Assert().NoError(err)
	...
	for _, ticket := range tickets {
		ticket.Username = tickets[0].Username
	}
	...
	resTickets, err := s.repo.GetAllByUserName(tickets[0].Username)
	t.Assert().NoError(err)
	t.Assert().Equal(ticketsPtr, resTickets)
}
