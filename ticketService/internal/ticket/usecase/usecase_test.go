...
func (s *TicketTestSuite) TestGetAll(t provider.T) {
	...
	ticketsUserName := make([]models.Ticket, len(tickets))
	for i, ticket := range tickets {
		ticketsUserName[i] = ticket
		ticketsUserName[i].Username = tickets[0].Username
	}

	ticketsUserNamePtr := make([]*models.Ticket, len(ticketsUserName))
	for i, ticket := range ticketsUserName {
		ticketsUserNamePtr[i] = &ticket
	}

	s.ticketRepoMock.On("GetAll").Return(ticketsPtr, nil)
	s.ticketRepoMock.On("GetAllByUserName", tickets[0].Username).Return(ticketsUserNamePtr, nil)

	cases := map[string]struct {
		TicketsPtr []*models.Ticket
		UserName   string
		Error      error
	}{
		"success all": {
			TicketsPtr: ticketsPtr,
			UserName:   "",
			Error:      nil,
		},
		"success userName": {
			TicketsPtr: ticketsUserNamePtr,
	...
	for name, test := range cases {
		t.Run(name, func(t provider.T) {
			resTickets, err := s.uc.GetAll(test.UserName)
			t.Assert().ErrorIs(err, test.Error)
			t.Assert().Equal(test.TicketsPtr, resTickets)
		})
	}
}
